...
data <- rename(data, state = State) %>% # rename column to tidy data format
    mutate(state.abb = state.abb) # Add states column. Orderis the same as in state.abb
    
data <- data %>% 
    gather(key = 'year', value = 'tuition', -c(state, state.abb)) # Align dataset to tidy format
